import {
  ...
  Bitcoin,
  Bitcoincash,
  Transaction as CodexTransaction,
  ContentType,
  Cosmos,
  Dogecoin,
  Ens,
...
/**
 * Configuration options for the CodexSDK class.
 * CodexSDK - A comprehensive SDK for blockchain interactions and authentication.
 *
...
export interface CodexSDKConfig {
  /**
   * The API key to use for authentication with the Codex API.
   */
  apiKey?: string;

  /**
   * An instance of the Supabase client to use for authentication with the Codex API.
   */
  authInstance?: SupabaseClient;

  /**
   * Configuration options for the HTTP client used by the CodexSDK class.
   */
  httpParams?: ApiConfig;

  /**
   * An instance of the HttpClient class to use for making HTTP requests to the Codex API.
   */
...
/**
 * A class that provides an interface to interact with various blockchain networks and services using the Codex API.
 */
export class CodexSDK extends EventEmitter {
  ...
  /**
   * Creates a new instance of the CodexSDK class.
   * @param config Configuration options for the CodexSDK class.
   */
  constructor(config?: CodexSDKConfig) {
    super();
    this.config = config || ({} as CodexSDKConfig);
    if (config) {
      this.config = config;
    }
    if (config && config.authInstance) {
      this.CodexAPIClient = config.authInstance;
    }
    if (config && config.apiKey) {
      this.CodexAPIClient = createClient(
    ...
    this.CodexAPIClient.auth.onAuthStateChange((event, session) => {
      switch (event) {
  ...
  /**
   * Returns the instance of the HttpClient class used by the CodexSDK class.
   */
  ...
  public async connect(
    ...
    try {
      if (accessToken && refreshToken) {
        this.CodexAPIClient.auth.setSession({
        ...
        const { data, error } = await this.CodexAPIClient.auth.getSession();
        if (data) {
          this.http.setSecurityData({
  ...
  public async disconnect(): Promise<void> {
    try {
      this.CodexAPIClient.auth.signOut();
      this.isAuthenticated = false;
      this.http.setSecurityData({
        token: '',
      });
  ...
  public getCodexAuth(): SupabaseClient {
    return this.CodexAPIClient;
  }
  ...
  /**
   * Returns a list of Ethereum accounts managed by the Codex API.
   */
  ...
  /**
   * Converts a CodexTransaction object to an array of TransactionData objects.
   * @param tx The CodexTransaction object to convert.
   */
  ...
  public async getChains(): Promise<Chains[]> {
    try {
      const { data, error } = await this.CodexAPIClient.from('chains').select(
        '*'
      );
      if (error) {
        throw new Error(error.message);
  ...
  public async getChainById(id: string): Promise<Chains> {
    try {
      const { data, error } = await this.CodexAPIClient.from('chains')
        .select('*')
        .eq('chain_id', id);
      if (error) {
        throw new Error(error.message);
  ...
  public async sendMagicLink(email: string, redirectTo: string) {
    const { error } = await this.getCodexAuth().auth.signInWithOtp({
      email: email,
      options: {
  ...
  public async signUp(email: string, password: string) {
    const { error } = await this.getCodexAuth().auth.signUp({
      email,
      password,
    });
    if (error) {
  ...
  public async signInWithPassword(email: string, password: string) {
    const { error } = await this.getCodexAuth().auth.signInWithPassword({
      email,
      password,
    });
    if (error) {
