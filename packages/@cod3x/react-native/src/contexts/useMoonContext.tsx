...
export const CodexContext = createContext<CodexContextProps>({
  user: null,
  session: null,
  initialized: false,
  moon: null,
...
export const CodexContextProvider = ({ children }: CodexProviderProps) => {
  const [user, setUser] = useState<User | null>(null);
  const [session, setSession] = useState<Session | null>(null);
  const [initialized, setInitialized] = useState<boolean>(false);
  const moon = new CodexSDK({ authInstance: supabase });
  const ethers = new CodexProvider({
    SDK: moon,
    chainId: 1,
  });
