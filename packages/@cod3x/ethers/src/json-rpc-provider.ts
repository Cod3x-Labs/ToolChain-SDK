...
import { CodexSigner } from './signer';
import { CodexProviderOptions } from './types';
...
export class JsonRpcProvider {
  ...
  private signer: CodexSigner;
  private config: CodexProviderOptions;
  ...
  public getSigner(): CodexSigner {
    return this.signer;
  }

  public setSigner(signer: CodexSigner): void {
    this.signer = signer;
  }

  public getSDK(): CodexSDK {
    return this.sdk;
  }

  public setSDK(sdk: CodexSDK): void {
    this.sdk = sdk;
  }

  public getConfig(): CodexProviderOptions {
    return this.config;
  }

  public setConfig(config: CodexProviderOptions): void {
    this.config = config;
  }
  ...
  public async updateConfig(options: CodexProviderOptions) {
    this.chainId = options.chainId;
    const chain = await this.sdk.getChainById(this.chainId.toString());
    const rpcUrls = chain.rpc_urls as string[];
